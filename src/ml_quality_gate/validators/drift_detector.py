...
Supports: PSI, KS test, Chi-squared.
"""
...
class DriftReport:
    ...
    def summary(self) -> dict[str, Any]:
        summary = {
            "dataset_drift": bool(self.dataset_drift),
            "drift_share": float(self.drift_share),
            "total_features": int(len(self.results)),
    ...
    def print_report(self) -> None:
        ...
        logger.info(
            f"Drift Share: {self.drift_share:.0%} "
            f"({len(self.drifted_features)}/{len(self.results)} features)"
        )
        logger.info(f"{'='*60}\n")
...
class DriftDetector:
    ...
    def _chi_squared_test(self, feature: str) -> DriftResult:
        ...
        chi2, p_value = stats.chisquare(
            cur_aligned,
            f_exp=ref_aligned * (cur_aligned.sum() / ref_aligned.sum()),
        )

        threshold = self._cfg.get("target_drift_p_value", 0.05)
    ...
    def detect_all(self, feature_columns: list[str] | None = None) -> DriftReport:
        ...
        report.dataset_drift = bool(report.drift_share >= drift_share_threshold)
